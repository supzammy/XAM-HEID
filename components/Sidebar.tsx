...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-80 bg-brand-surface p-6 flex-shrink-0 flex flex-col space-y-8 overflow-y-auto">
      <div>
        <h2 className="text-lg font-semibold text-brand-light mb-4">Filters</h2>
        
        <div className="space-y-6">
          <div>
            <div className="flex justify-between items-center mb-3">
                <h3 className="text-sm font-medium text-gray-400">Year</h3>
                <span className="text-lg font-bold text-brand-teal">{selectedYear}</span>
            </div>
            <input
                type="range"
                ...
                className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-brand-teal"
            />
          </div>

          <div>
            <h3 className="text-sm font-medium text-gray-400 mb-3">Health Conditions</h3>
          ...
          <div>
            <h3 className="text-sm font-medium text-gray-400 mb-3">Demographics</h3>
            <div className="space-y-2">
              {DEMOGRAPHICS.map((demographic) => (
                <div
                  key={demographic}
                  onClick={() => setSelectedDemographic(demographic)}
                  className={`flex items-center p-3 rounded-md cursor-pointer transition-colors duration-200 bg-gray-700 bg-opacity-40 hover:bg-gray-600 ${selectedDemographic === demographic ? 'ring-2 ring-brand-teal' : ''}`}
                >
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${selectedDemographic === demographic ? 'border-brand-teal' : 'border-gray-400'}`}>
                    {selectedDemographic === demographic && <div className="w-2 h-2 rounded-full bg-brand-teal"></div>}
                  </div>
                  <span className="ml-3 text-sm font-medium text-brand-light">{demographic}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {subCategories.length > 0 && selectedDemographic && (
            <div className="border-t border-gray-700 pt-4">
              <h3 className="text-sm font-medium text-gray-400 mb-3">Sub-Category for {selectedDemographic}</h3>
              
              {isAgeDemographic ? (
                <div>
                    <div className="flex justify-end items-center mb-3">
                        <span className="text-lg font-bold text-brand-teal">{selectedDemographicDetail}</span>
                    </div>
                    <input
                        type="range"
                        min="0"
                ...
                <div className="space-y-2">
                  {subCategories.map((detail) => (
                    <div
                      ...
                      className="flex items-center p-3 rounded-md cursor-pointer transition-colors duration-200 bg-gray-800 bg-opacity-60 hover:bg-gray-700"
                    >
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${selectedDemographicDetail === detail ? 'border-brand-teal' : 'border-gray-500'}`}>
       ...
       <div className="mt-auto pt-6 border-t border-gray-700">
        <h3 className="font-semibold text-brand-teal mb-2 text-sm">Data & Privacy Notice</h3>
        <div className="text-xs text-gray-400 space-y-2 p-3 bg-black bg-opacity-20 rounded-md">
          <p>
